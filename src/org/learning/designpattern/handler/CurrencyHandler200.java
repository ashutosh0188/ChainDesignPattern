...
/**
 * A class that perform counting currency of 200.
 * @author Ashutosh Srivastava
 */
public class CurrencyHandler200 implements Chain {
    private Chain chain;

    /**
     * This method checks how much max currency if 200 can be dispensed to withdraw cash.
    ...
    public void process(int amount, DispenseCurrency dispenseCurrency) {
        int requiredCurrency  = amount/200;
        int availableCurrency = ATMCash.getInstance().getCurrency200();
        int remainingAmount;
        ...
        dispenseCurrency.setCurrency200(actualCurrencyDispensed);
        ATMCash.getInstance().setCurrency200(remainingCurrency);
        remainingAmount = amount - (actualCurrencyDispensed*200);
