...
/**
 * A class that perform counting currency of 1000.
 * @author Ashutosh Srivastava
 */
public class CurrencyHandler1000 implements Chain {
    private Chain chain;

    /**
     * This method checks how much max currency if 1000 can be dispensed to withdraw cash.
    ...
    public void process(int amount, DispenseCurrency dispenseCurrency) {
        int requiredCurrency  = amount/1000;
        int availableCurrency = ATMCash.getInstance().getCurrency1000();
        int remainingAmount;
        ...
        dispenseCurrency.setCurrency1000(actualCurrencyDispensed);
        ATMCash.getInstance().setCurrency1000(remainingCurrency);
        remainingAmount = amount - (actualCurrencyDispensed*1000);
